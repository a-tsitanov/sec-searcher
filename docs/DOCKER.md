...
Текущие настройки: контекст 65 536, максимальный ответ агента 16 384, таймаут запроса 720 секунд, входной профиль истории агента 32 768. Для снижения расхода памяти включены KV-кэш Q8 (`q8_0` для K/V) и Flash Attention. Бюджет анализа остаётся 100 шагов / 1800 секунд.

Проверено на M4 Pro: `/props` вернул `n_ctx=65536`; запрос из контейнера с `max_tokens=16384` завершился ответом `OK.` и `finish_reason=stop`. Все 53 теста прошли. Полная генерация 16K и качество аудита с Q8-кэшем отдельно не оценивались. После обновления существующего Metal-процесса выполните `bash scripts/metal.sh stop`, затем `bash install.sh --backend metal --no-download`; повторный старт со старым контекстом отклоняется.
